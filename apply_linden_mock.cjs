...
function applyLindenRanges(dataObj) {
  if (dataObj && dataObj.daily && Array.isArray(dataObj.daily)) {
    ...
    // UI Linden butuh data HARIAN yang dipotong sesuai rentang
    const wArr = JSON.parse(JSON.stringify(dArr)).slice(-7);
    const mArr = JSON.parse(JSON.stringify(dArr)).slice(-30);
    const yArr = dataObj.yearly || [];
    
    // Gantikan array-nya
    dataObj.daily = dArr.slice(-2); // Pakai 2 hari agar % jalan
    dataObj.weekly = wArr;
...
// Terapkan ke file final
fs.writeFileSync(bundlePath, JSON.stringify(data, null, 2), 'utf8');
console.log('✅ RULES LINDEN DITERAPKAN DENGAN SEMPURNA PADA DATA BARU!');
