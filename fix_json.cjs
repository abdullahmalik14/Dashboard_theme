    ...
    obj.weekly.forEach((item, i) => {
      if (item.period && String(item.period).includes('-')) {
         ...
         if (!isNaN(d.getTime())) {
           ...
           item.period = `${d.getFullYear()}-W${w.toString().padStart(2, '0')}`;
         }
      }
    });
  }
  ...
  if (Array.isArray(obj.monthly)) {
    obj.monthly.forEach((item, i) => {
      if (item.period && String(item.period).includes('-')) {
         const d = new Date(item.period);
         if (!isNaN(d.getTime())) {
           item.period = `${d.getFullYear()}-${(d.getMonth()+1).toString().padStart(2, '0')}`;
         }
      }
    });
  }
...
if (data.subscriptions && Array.isArray(data.subscriptions.yearly)) {
  const existing = data.subscriptions.yearly.find(y => y.period === "2026");
  if (existing) {
     const mockYear = {
        ...existing,
        period: "2025",
